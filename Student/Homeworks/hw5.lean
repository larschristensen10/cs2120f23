...
def not_either_not_both { jam cheese } :
  ((no jam) ⊕ (no cheese)) → 
  (no (jam × cheese)) 
| (Sum.inl nojam), (a, b) => nojam a
| (Sum.inr nocheese), (a, b) => nocheese b
...
| (Sum.inl noa), (a, b) =>  noa a
| (Sum.inr nob), (a, b) => nob b
...
| noaorb => (λ a => noaorb (Sum.inl a), λ b => noaorb (Sum.inr b))
...
#eval nat_le 5 4 -- expect false
#eval nat_le 4 4 -- expect true
#eval nat_le 4 5 -- expect true
#eval nat_le 5 5 -- expect true
